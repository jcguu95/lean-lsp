...
## Commands
All commands use `./lean-lsp`.

### Server Commands (run on the host)
- Start the daemon:
  - `cd /path/to/lean/project && ../lean-lsp start --host 0.0.0.0`
- Check daemon status:
  - `./lean-lsp check --host <host>`
- Stop the daemon:
  - `./lean-lsp stop --host <host>`

### Client Commands (run by the agent, e.g., in Docker)
These commands require `--host`, and usually path mapping flags.

- Hover info:
  - `./lean-lsp hover --host <host> --map-root-from <client_path> --map-root-to <host_path> <file> <line> <col>`
- Plain goal:
  - `./lean-lsp plain-goal --host <host> --map-root-from <client_path> --map-root-to <host_path> <file> <line> <col>`
- Diagnostics at a position:
  - `./lean-lsp diagnostics --host <host> --map-root-from <client_path> --map-root-to <host_path> <file> <line> <col>`
- All of the above:
  - `./lean-lsp all --host <host> --map-root-from <client_path> --map-root-to <host_path> <file> <line> <col>`
- Raw LSP request:
  - `./lean-lsp request --host <host> --method <method> --params <json>`
