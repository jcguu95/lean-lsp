...
### Added
- (Untested) Introduced `bin/lean-exec`, a secure command proxy to run `lean` and `lake` from a container on the host. This is a potential solution to CPU architecture mismatch errors (`Exec format error`).
- (Untested) Updated `aider/` setup to use `lean-exec`. The Dockerfile now creates a minimal image with command wrappers, and `aider/README.md` documents the new two-terminal setup.

## [0.6.2] - 2026-02-09

### Changed
- Refactored `CHANGELOG.md` into a main file with links to version-specific files in the `changelog/` directory.
- Improved `aider/README.md` with clear, step-by-step instructions for building the Docker image and running the Aider container. The `docker run` command now uses a commented Bash array for better readability.
