...
### Changed
- Removed `--verbose` flag from `lean-lsp start` in `test-lean-lsp.sh` to produce cleaner test output.

## [0.6.15] - 2026-02-09

### Changed
- Refactored `bin/test-lean-lsp.sh` to use a temporary, minimal Lean project for testing instead of the checked-in `test-project`. This makes the test suite cleaner and removes the `mathlib` dependency for tests.

## [0.6.14] - 2026-02-09

### Fixed
- The `lean-lsp` client now correctly resolves the server address `0.0.0.0` to `127.0.0.1` for connections. This allows the daemon to bind to all network interfaces while client commands (`check`, `request`, etc.) can successfully connect to it.

## [0._6.13] - 2026-02-09

### Changed
- Moved the `COPY bin` instruction in `agent/Dockerfile` to a later build stage. This improves Docker layer caching, preventing costly reinstalls of the Lean toolchain when only local scripts are modified.

## [0.6.12] - 2026-02-09

### Changed
- Clarified in `README.md` that client commands are run from the repo root and added a tip pointing to the Docker-based agent setup in the `agent/` directory.
